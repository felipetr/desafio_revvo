    ...
    xhr.onload = function() {
      if (xhr.status === 200) {
        ...
        if (response.success) {
          ...
          document.getElementById("loginMessage").innerHTML = "Erro no login: " + response.message;
        }
      } else {
        console.error("Erro na requisição AJAX:", xhr.statusText);
        document.getElementById("loginMessage").style.display = "block";
        document.getElementById("loginMessage").innerHTML = "Erro no servidor. Tente novamente mais tarde.";
      }
    };
